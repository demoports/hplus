...
// fn_28ed8(eax, ebp), fn_2afd3(ebx, esi, edi, ebp) camera setup, fn_2af3a(), fn_2b037(ebx, ecx) spline walker advance,
...
// this part's own globals (0x178c8..0x180ec)
const PART = Object.freeze({
  exitOrders : 0x178c8,     // orders to run (eax of the run loop)
  exitRow    : 0x178cc,     // row to stop at (ebx of the run loop)
  startOrder : 0x178d0,
  frameBuf   : 0x178d8,     // 0xb6010 alloc, 16-aligned
  frameBuf2  : 0x178dc,     // second page, frameBuf + 0x4b000
  scaleOne   : 0x178e0,     // 1.0f bits, the scale handed to buildObject
  tmpI       : 0x178e8,     // int spill slot: the original stored here; the port keeps the store so memory snapshots match
  tickCounter: 0x17eb8,     // incremented per update step, used & 3
  curOrder   : 0x17ebc,
});
...
HP.fn_1804a = function (esi, edi, ebp) {
  for (let c = 0x40; c > 0; c--) {
    let eax = (rand(0x320) - 0x190) | 0;
    wr32(PART.tmpI, eax); wrf(edi + 8, eax);
    eax = (rand(0x28) + ebp) | 0;
    wr32(PART.tmpI, eax); wrf(edi + 4, eax);
    eax = (rand(0x28) + esi) | 0;
    wr32(PART.tmpI, eax); wrf(edi + 0xc, eax);
    wr32(PART.tmpI, -0xc); wrf(edi + 0x20, -12);
    edi += 0x2c;
  }
  return edi;
};
...
HP.fn_1816a = function () {
  ...
  wr32(PART.frameBuf, eax); eax += 0x4b000; wr32(PART.frameBuf2, eax); eax += 0x4b000;
  wr32(0x23fd0, 0x64);
  copyChunk(0x161d4, rd32(PART.frameBuf));
  buildObject(0, 1, 0, 0xffffffff, rd32(PART.scaleOne), rd32(PART.frameBuf), 0x178ec);     // (eax, ebx, ecx, edx, esi, edi, ebp) — engine signature
  wr32(0x23fd0, 0x9c4);
  copyChunk(0x171db, rd32(PART.frameBuf));
  buildObject(3, 9, 0, 0xffffffff, rd32(PART.scaleOne), rd32(PART.frameBuf), 0x178ec);
  wr32(0x17914, 1); wr32(0x17918, 0x1a); wr32(0x17919, 0x1a); wr32(0x1791a, 0x28);   // overlapping dword stores, as in the original
  HP.fill32(rd32(PART.frameBuf), 0, 0x25800);
  objectInit(0xc0, 0x178ec);          // (ecx, ebp)
  ...
  wrf(0x17d58, 1); wr32(PART.tmpI, 0xff); wrf(0x17d5c, 255);
...
HP.fn_184f0 = function () {
  ...
  stirRng();
  splineAdvance(0x17e68, rd32(PART.scaleOne));        // (ebx=angles, ecx)
  HP.fn_180ec(8, rd32(0x17e00));              // (ebp, edi)
  const esi = rd32(ADDR.playerState);
  const eax = rd32(esi + 0x30);
  if (eax !== rd32(PART.curOrder)) {
    wr32(PART.curOrder, eax);
    wr32(PART.tickCounter, rd32(PART.tickCounter) + 1);
    const ebx = rd32(PART.tickCounter) & 3;
    if (rd32(ebx * 4 + 0x17ee0) !== rd32(ebx * 4 + 0x17edc)) {
      const s = rd32(PART.tickCounter) & 3;
...
HP.fn_182fc_init = function (eax, ebx) {
  wr32(PART.exitOrders, eax); wr32(PART.exitRow, ebx);
  const esi = rd32(ADDR.playerState);
  const startOrder = rd32(esi + 0x30);
  wr32(PART.curOrder, startOrder); wr32(PART.startOrder, startOrder);
  wr32(PART.tickCounter, 0);
  const s = rd32(PART.tickCounter);
  wr32(0x17e6c, rd32(s * 4 + 0x17ed0)); wr32(0x17e74, rd32(s * 4 + 0x17ec0));
  wr32(0x17e70, 0); wrf(0x17e68, 0);
  wr32(ADDR.timerMs, 0);
  HP.fill32(rd32(PART.frameBuf), rd32(rd32(0x17d50)), 0x12c00);
};
...
HP.fn_182fc_render = function () {
  const path = rd32((rd32(PART.tickCounter) & 3) * 4 + 0x17ee0);
  wr32(0x28c9c, 1);
  cameraFromSpline(0x17e68, 0x17e04, path, 0x178ec);     // (ebx=0x17e68, esi=0x17e04, edi=path, ebp=0x178ec)
  ...
  renderObject(0x17e04, 0x178ec, rd32(PART.frameBuf));     // (esi=camera, ebp=object, edi=buffer) — engine signature order
  HP.fn_182fc_blend();
  HP.fn_18135(rds32(0x17e8c), rds32(0x17e90), rd32(PART.frameBuf));   // (ecx, edx, edi)
};
...
HP.fn_182fc = function* (eax, ebx) {
  ...
  for (;;) {
    HP.fn_182fc_render();
    present(rd32(PART.frameBuf), 0x178ec);              // present (esi=buffer, ebp=object)
    yield;
    wr32(0x178d4, 2);
    ...
    if (rds32(esi + 0x30) < (rds32(PART.startOrder) + rds32(PART.exitOrders) | 0)) continue;
    if (rds32(esi + 0x34) < rds32(PART.exitRow)) continue;
    return;
  }
};

...
HP.fn_1545e = function (edx, ebp, esi, edi) {
  for (let c = 0x40; c > 0; c--) {
    let eax = rand(0x1770) + 0xe6;
    if (edx === 1) eax = -eax;
    wr32(0x14530, eax); wrf(edi + ebp, eax);
    eax = rand(0x14) - 10; wr32(0x14530, eax); wrf(edi + esi, eax);
    eax = rand(0x14) - 10; wr32(0x14530, eax); wrf(edi + 0xc, eax);
...
HP.fn_1579c = function () {
  const M = HP.M;
  wr32(0x143e5, 1);
  let cl = (rand(3) + 5) & 0xff;
  if (rand(0x28) === 6) cl = 2;
  let esi = rd32(0x143f0), edi = rd32(0x143f8);
  let ebp = rand(0xffffffff), ebx = rand(0xffffffff);
  for (let n = 0x12c00; n > 0; n--) {
    if (M[esi] !== 1) {
...
HP.fn_15922 = function () {
  let eax = alloc(0x96010); eax = (eax | 0xf) + 1;
  wr32(0x1451c, eax); eax += 0x4b000; wr32(0x14520, eax); eax += 0x4b000;
  let edi = HP.fn_156ff(0xe474, rd32(0x1451c));
  buildObject(1, 1, 0, 0xffffffff, rd32(0x14528), rd32(0x1451c), 0x14544);   // (eax, ebx, ecx, edx, esi, edi, ebp) — engine signature
  edi = HP.fn_156ff(0xea3c, rd32(0x1451c));
  buildObject(1, 1, 0, 0xffffffff, rd32(0x14528), rd32(0x1451c), 0x14544);
  edi = rd32(0x1451c);
  ...
  wr32(edi, 0);
  buildObject(1, 0x101, 0, 0xffffffff, rd32(0x14528), rd32(0x1451c), 0x14a5c);
  edi = rd32(0x1451c);
  ...
  edi = HP.fn_15646(edi);
  wr32(edi, 0);
  buildObject(1, 0x101, 0, 0xffffffff, rd32(0x14ff8), rd32(0x1451c), 0x14a5c);
  ...
  objectInit(0x100, 0x14544);
  objectInit(0, 0x14a5c);
  ...
  eax = alloc(0x12c10); eax = (eax | 0xf) + 1; wr32(0x143f0, eax);
  HP.fill32(eax, 0, 0x4b00);
  HP.copy(rd32(0x143f0) + 0x76c0, 0x10564, 0xfa0 * 4);
  eax = alloc(0x2d10); eax = (eax | 0xf) + 1; wr32(0x143ec, eax);
  for (let i = 0; i < 0x1680; i++) HP.M[eax + i] = (HP.M[0xeee4 + i] << 4) & 0xff;
};

// fn_15fb0: one update step (14 ms)
HP.fn_15fb0 = function () {
  if (rd8(ADDR.keyPause) !== 1) {
    stirRng();
    splineAdvance(0x14fd8, rd32(0x14528));          // (ebx=angles, ecx=matrix)
  ...
  if (rd32(ADDR.glitchLevel) === 0) {
    if (rds32(0x14fe0) >= 0xf0) wr32(ADDR.glitchLevel, 1);
  }
  if (rd32(ADDR.glitchLevel) !== 0) {
    wr32(ADDR.glitchLevel, rd32(ADDR.glitchLevel) + 1);
  ...
  if (rd8(0x9ec) === 1) { wr32(ADDR.glitchLevel, 0x44c); wr32(0x144fc, 0); }
};
...
HP.fn_15c18 = function* (eax, ebx) {
  ...
  for (;;) {
    if (rds32(0x143e5) !== 1) {
      ...
      cameraFromSpline(0x14fd8, 0x14f74, edi, 0x14544);    // (ebx=0x14fd8, esi=0x14f74, edi=path, ebp=0x14544)
      wr32(0x14a90, rd32(0x14578));
      HP.fn_1570b(0x14500, 0x14f74);
      const sa = rd32(0x145ac), sb = rd32(0x145a4);
      wr32(0x145ac, 0); wr32(0x145a4, 0);
      renderObject(0x14f74, 0x14544, rd32(0x1451c));    // (esi=camera, ebp=object, edi=buffer) — engine signature order
      wr32(0x145a4, sb); wr32(0x145ac, sa);
      renderObject(0x14f74, 0x14a5c, rd32(0x1451c));
      renderObject(0x14f74, 0x14544, rd32(0x1451c));
    ...
    if (rds32(ADDR.glitchLevel) >= 0x44c) HP.fn_1579c();
    else {
      const b = rd32(0x143f8);
      HP.fill32(b, 0, 0x140);
      for (let y = 0, p = b; y < 0xf0; y++, p += 0x500) wr32(p, 0);
    }
    present(rd32(0x143f8), 0x14544);          // present (esi=buffer, ebp=object)
    yield;
    wr32(0x14518, 1);
    let n = Math.trunc((rds32(ADDR.frameMs) + 1) / 0xe);
    for (; n > 0; n--) HP.fn_15fb0();
    HP.fill32(rd32(0x1451c), 0, 0x12c00);
    if (rds32(ADDR.glitchLevel) > 0x7d0) return;
